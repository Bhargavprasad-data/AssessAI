...
export const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user.role !== 'admin') {
        setError(`Access Denied: You are logged in as a ${user.role}. This portal is restricted to system administrators.`);
        return;
      }
      navigate('/admin/dashboard');
    } catch (err: any) {
      setError(err.message || 'Login failed. Please verify administrator credentials.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md relative z-10 my-6">
        ...
        <div className="text-center mb-6">
          ...
          <p className="dark:text-slate-400 text-slate-500 text-xs sm:text-sm mt-1.5">
            Platform Infrastructure & Global Telemetry Control
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="admin@assessai.edu"
