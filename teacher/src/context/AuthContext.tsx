...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [user, setUser] = useState<User | null>(() => {
    try {
      ...
      return parsed && parsed.role === 'teacher' ? parsed : null;
    } catch {
      return null;
    }
  });
  ...
  const [loading, setLoading] = useState<boolean>(() => {
    try {
      ...
      const parsed = JSON.parse(cached);
      return !(parsed && parsed.role === 'teacher');
    } catch {
      return true;
    }
  });
    ...
    async function checkAuth() {
      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
          ...
          if (userData && userData.id && userData.role === 'teacher') {
            setUser(userData);
          ...
          if (refreshed) {
            ...
            if (isMounted && retryUser && retryUser.id && retryUser.role === 'teacher') {
              setUser(retryUser);
          ...
          // Not a valid teacher session on server
          if (isMounted) {
            setUser(null);
