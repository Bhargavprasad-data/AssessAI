...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [loading, setLoading] = useState<boolean>(() => {
    try {
      ...
      const parsed = JSON.parse(cached);
      return !(parsed && parsed.role === 'student');
    } catch {
      return true;
    }
  });
      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
          ...
          if (userData && userData.id && userData.role === 'student') {
            // Valid student session confirmed
            setUser(userData);
          ...
          // If null or another role was returned due to shared cookies, try explicit isolated refresh
          const refreshed = await attemptTokenRefresh();
          if (refreshed) {
            const retryUser = await apiFetch<User>('/api/auth/me');
            if (isMounted && retryUser && retryUser.id && retryUser.role === 'student') {
              setUser(retryUser);
          ...
          // Not a valid student session on the server; clear any mismatched cached credentials
          if (isMounted) {
            setUser(null);
          ...
          // Network down or other unexpected error
          if (isMounted) {
            setLoading(false);
          }
          return;
...
const defaultStudentAuthContext: AuthContextType = {
  user: null,
  loading: true,
...
export const useAuth = (): AuthContextType => {
  ...
  if (!context) {
    return defaultStudentAuthContext;
  }
  return context;
};
