...
interface NavItem {
  ...
  /** If true, match prefix instead of exact path */
  prefix?: boolean;
}

const NAV_ITEMS: NavItem[] = [
  { label: 'My Assessments', path: '/student/dashboard', icon: BookOpen },
  { label: 'My Results',     path: '/student/attempts', icon: Trophy, prefix: true },
];
...
export const Sidebar: React.FC = () => {
  ...
  return (
    <div className="relative flex-shrink-0 h-screen print:hidden" style={{ width: SIDEBAR_COLLAPSED }}>
      <motion.aside
        ...
        transition={SPRING}
        className={[
          'fixed top-0 left-0 h-screen z-50 overflow-hidden flex flex-col border-r print:hidden',
          isDark
      ...
      >
          ...
          <AnimatePresence>
            {open && (
              ...
              >
                ...
                <span className={['block text-[10px] font-semibold uppercase tracking-widest', isDark ? 'text-emerald-400' : 'text-emerald-600'].join(' ')}>
                  Student Portal
                </span>
              </motion.div>
            )}
            ...
            return (
              <Link
                ...
                className={[
                  ...
                  active
                    ? isDark ? 'bg-emerald-600/15 text-emerald-300' : 'bg-emerald-500/10 text-emerald-700'
              ...
              >
                ...
                <Icon className={['flex-shrink-0 w-5 h-5 transition-colors', active ? 'text-emerald-500' : isDark ? 'text-slate-500 group-hover:text-white' : 'text-slate-400 group-hover:text-slate-700'].join(' ')} />
                <AnimatePresence>
                  {open && (
                    <motion.span
        ...
        <div className={['border-t px-2 py-3 space-y-0.5', isDark ? 'border-white/8' : 'border-slate-100'].join(' ')}>
          ...
          {/* User + Logout */}
          <div className={['flex items-center gap-3 px-2.5 py-2 rounded-xl transition-colors group', isDark ? 'hover:bg-white/5' : 'hover:bg-slate-50'].join(' ')}>
            <div
            ...
            <AnimatePresence>
              {open && (
                ...
                >
                  ...
                  <p className="text-[10px] font-medium uppercase tracking-wider text-slate-400">Student</p>
                </motion.div>
              )}
            </AnimatePresence>
            <AnimatePresence>
