  ...
  refreshPromise = (async () => {
    try {
      ...
      const res = await fetch(getApiUrl('/api/auth/refresh'), {
        ...
        credentials: 'include',
        headers,
        body: storedRefreshToken ? JSON.stringify({ refresh_token: storedRefreshToken, role: 'student' }) : undefined,
      });
      if (res.ok) {
        ...
        // Protect student portal against cross-portal cookie contamination
...
export async function apiFetch<T>(
  ...
  if (!headers.has('X-Expected-Role')) {
    headers.set('X-Expected-Role', 'student');
  }
